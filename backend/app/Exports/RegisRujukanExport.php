...
class RegisRujukanExport implements FromQuery, 
    ...
    WithHeadings
{
   ...
   public function query()
   {
        ...
        $query = PasienRegister::leftJoin('register','register.id','pasien_register.register_id')
                    ...
                    ->join(DB::raw("(select register_id,string_agg(sumber_sampel,';') sumber_sampel from pengambilan_sampel
                    LEFT JOIN sampel on sampel.pengambilan_sampel_id = pengambilan_sampel.id
                    group by register_id) sumber"), function($join){
                        $join->on('sumber.register_id','pasien_register.register_id');
                    })
                    ->select([
        ...
        $query->where('jenis_registrasi','rujukan');
        return $query;
   }
