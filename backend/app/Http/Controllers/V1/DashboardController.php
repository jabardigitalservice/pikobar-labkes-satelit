...
class DashboardController extends Controller
{
    ...
    {
        $count_by_status = Register::groupBy('register_status')->select('register_status', DB::raw('count(*) as total'))->pluck('total','register_status');
        $registration_incomplete = Register::whereNull('waktu_waiting_sample')->count();
        $tracking = [
    ...
    public function ekstraksi()
    {
        ...
        $count_by_labs = Register::groupBy('lab_pcr_nama')->orderByRaw('count(*) desc')
            ->select('lab_pcr_nama', DB::raw('count(*) as total'))
            ->whereIn('register_status',['extraction_sample_sent','pcr_sample_received','pcr_sample_analyzed','sample_verified','sample_valid'])
            ->get();
